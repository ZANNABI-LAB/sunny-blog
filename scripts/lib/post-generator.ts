...
### tags 규칙
- 반드시 **영어**, **kebab-case**로 작성 (예: "design-pattern", "web-security")
- 3~5개 작성
- 한국어 태그 절대 금지

### references 규칙
- 공식 문서, MDN, RFC 등의 URL을 2~3개 포함
- 실제로 존재하는 URL만 사용
- maeil-mail.kr URL은 references에 포함하지 마세요
- 예: ["https://developer.mozilla.org/...", "https://docs.spring.io/..."]
...
const validateFrontmatter = (
  ...
  try {
    ...
    // Category validation: root must be one of VALID_CATEGORIES
    const categoryRoot = (data.category as string).split(".")[0];
    if (!VALID_CATEGORIES.includes(categoryRoot as (typeof VALID_CATEGORIES)[number])) {
      console.warn(`[생성기] 유효하지 않은 카테고리: ${data.category} (root: ${categoryRoot})`);
      return null;
    }

    // References validation
    const rawReferences = Array.isArray(data.references) ? (data.references as string[]) : [];
    if (rawReferences.length > 0 && !rawReferences.every((r) => typeof r === "string" && r.startsWith("http"))) {

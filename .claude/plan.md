...
- PRD-84: 공용 컴포넌트/레이아웃 통일 — PageHeader 신설(복붙 4곳 대체), 컨테이너 폭 2종(`max-w-reading`/`max-w-wide`) 표준화, `.prose-blog` @utility로 tech/log/privacy 본문 통일, tab-nav 워드마크·활성 판정 중복 제거, `useIsMobile`/`usePrefersReducedMotionRef` 훅 추출

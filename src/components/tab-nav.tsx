  ...
  return (
    <header className="sticky top-0 z-50 bg-[#0a0a0f]/80 backdrop-blur-sm border-b border-white/10">
      <nav aria-label="메인 네비게이션" className="flex items-center gap-1 max-w-5xl mx-auto px-4 h-12">
        <Link href="/" className="font-bold text-white mr-4 shrink-0 rounded-sm text-glow-indigo-sm focus-visible:ring-2 focus-visible:ring-indigo-400/50 focus-visible:outline-none">
          Sunny&apos;s Blog
        </Link>
        {tabs.map(({ href, label }) => {
          ...
          return (
            <Link
              ...
              style={isActive ? { textShadow: '0 0 8px rgba(99,102,241,0.3)' } : undefined}
            >
              {label}
            </Link>
          );
